...
class InterviewAgents:
    ...
    def generate_question(self, elements, messages, message):
        ...
        4. 必ず回答者の**現在のタスクに関する質問**のみを生成してください\n
        ...
        if all(len(v) >= 2 for key, v in elements.items()):  # 条件に基づいて分岐
            # もしすべての要素が2つ以上埋まっている場合、別のエージェントを使用
            prompt = f"""
            インタビュー履歴: {messages}
            最新の回答: {message}

            質問技法を使って、これまでの会話に基づいて回答者の仕事における信念や哲学、本音を引き出す深堀の質問をこれまでの会話内容と絡めて生成してください。質問技法は、仮説を提示したり、具体物を提示したり、主観的意見を提示したり、共感を示したり、後押しをするなどを用いてください。
            ただし、信念や哲学は何ですかとダイレクトに聞くのではなく、相手が自然に話しやすいように工夫してください。
            """
            ...
            ただし、手がかりという言葉は使わず、なるべく簡潔で分かりやすい質問を生成してください。
            """
            return self._get_gpt_response(system_message, prompt)
        # prompt = f"""
    ...
    def check_if_interview_should_end(self, messages, elements) -> bool:
        ...
        sufficient_elements = all(
            len(v) >= 1 if key == "行動" else len(v) >= 2 for key, v in elements.items()
        )
    ...
    def extract_elements(self, message, messages, elements):
        ...
        行動は日常性や内容、時間的圧力などの情報が含まれ、認知は気付きや手がかり、選択肢や目標などの情報が含まれ、情報は、行動において用いる知識や経験などの参考にしていることが含まれます。また、その人の本音や信念に関しての回答は、行動、認知、情報のどれに関係するかを考慮して分類してください。
    ...
    def check_question(
        ...
        初めて聞く内容の質問であれば適切、履歴に類似した内容の質問があれば不適切とし、インタビュー履歴と最新の回答を参考にして、質問が新しい内容で、似た質問をそれまでに生成していないかどうか判断し、質問として適切かどうかを確認してください。
