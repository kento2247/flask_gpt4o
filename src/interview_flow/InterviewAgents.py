...
class InterviewAgents:
    ...
    def generate_question(self, elements, messages, message):
        ...
        質問生成における制約は、以下の通りです。\n
        1. 質問は一回につき最大一個にしてください。{message}を考慮した簡単な相槌も生成してください。\n
        2. 重複した質問を避けてください・\n
        ...
        prompt = f"""
        それまでのインタビューの対話{messages}と直前の回答である{message}を参照し、簡単な相槌と一つの質問を生成してください。
    ...
    def interview_question(self, messages, message):
        ...
        インタビュー履歴: {messages}
        直前の回答: {message}
        現在の作業者のタスクについての認知プロセスと仕事に対する回答者の信念や本音という二つの要素を簡潔に聞き出してください。
        """
        return self._get_gpt_response(system_message, prompt)
