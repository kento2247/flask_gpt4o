...
class InterviewAgents:
    ...
    def gpt_generate_question(self, messages, message):
        system_message = """
        ...
        2.誘導的でない質問
        3.オープンエンドな質問
        4. 本音や、背景にある意味や動機を引き出すために、仮説の提示,具体物を提示,主観的意見の提示,共感,後押しを活用する。\n
        終了条件:インタビューガイドについての項目の回答がある程度得られた場合インタビューを終了し、また相手がインタビューを退屈に感じたり不快に感じたりしている場合は、質問内容を変えるか、インタビューを終了してください。
        """
        prompt = f"""
        インタビューガイドは以下のinterview_purposeとquestion_itemsです。
        interview_purpose: "従業員のあるタスクに対する認知プロセスとその背景にある信念を理解すること"
        question_items:
        - "対象とするタスク内容(日常性や行動の内容)"
        - "対象とするタスクを行う際の認知プロセス(優先度や目標、手がかり)"
        - "対象とするタスクにを行う際に活用している情報(経験や知識)"
        メッセージ履歴: {messages}
        直前の回答: {message}
        半構造化インタビューのプロとして、簡潔かつ短いフレーズによるインタビューを行ってください。相手の言っていることの意味を確認したり、深堀して本音を引き出すことも意識してください。
        インタビューガイドの項目に関しての回答が得られたらインタビューを終了してください。必ず一文にしてください。1対話につき一つの質問のみを生成してください。
        """
