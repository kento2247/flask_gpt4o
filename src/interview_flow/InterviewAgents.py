...
class InterviewAgents:
    ...
    def generate_question(self, elements, messages, message):
        ...
        if sum(len(v) for v in elements.values()) >= 4:
            ...
            また、まだ深堀ができるような最新の回答であれば、それについて深堀の質問を生成してください。例えば、「高品質なミルクを作る」という最新の回答があれば、それについて「高品質なミルクを作ることに対して、どのようなこだわりを持っていますか？」などの質問を生成してください。具体例は実際に使わないこと。
            ...
            共感を示す\n
            後押しをする\n
            また、必ず現在の回答者のタスクに関する質問のみを生成してください。
